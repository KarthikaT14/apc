...
def run_prediction_app():
    ...
    if uploaded_file is not None:
        ...
        if DATE_COL in df_display.columns:
            df_display[DATE_COL] = pd.to_datetime(df_display[DATE_COL], errors="coerce", dayfirst=True)
            df_display[DATE_COL] = df_display[DATE_COL].dt.strftime('%Y-%m-%d').fillna('')
        ...
        try:
            ...
            if DATE_COL in final_output.columns:
                 final_output[DATE_COL] = pd.to_datetime(final_output[DATE_COL], errors="coerce", dayfirst=True)
                 final_output[DATE_COL] = final_output[DATE_COL].dt.strftime('%Y-%m-%d').fillna('')
